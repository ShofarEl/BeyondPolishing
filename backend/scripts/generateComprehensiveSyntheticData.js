...
// The 30 seed problems (exactly as provided)
const seedProblems = [
...
const generateUser = (index) => {
  return {
    sessionId: `study_participant_${String(index + 1).padStart(3, '0')}_${uuidv4().substring(0, 8)}`,
    studyGroup: index % 2 === 0 ? 'editor-first' : 'challenger-first',
    consentGiven: true,
    consentTimestamp: generateTimestamp(-1, Math.floor(Math.random() * 12) + 8), // Day before, random time
    demographicData: {
...
const generateRating = (promptType, responseQuality = 'good') => {
  ...
  const variation = (baseValue) => Math.max(1, Math.min(5, Math.round(baseValue + (Math.random() - 0.5) * 2)));

  return {
    usefulness: variation(base.usefulness),
    cognitiveLoad: variation(base.cognitiveLoad),
    satisfaction: variation(base.satisfaction)
  };
};
...
const generateFeedback = (promptType, rating) => {
  const editorFeedbacks = [
    ...
    "Excellent guidance on making the problem more measurable. The success criteria suggestions were spot-on.",
    "Really appreciated the focus on scope and constraints. Helped me think more realistically about the project."
  ];
  ...
  const challengerFeedbacks = [
    ...
    "The community-driven solutions angle was unexpected but compelling. Made me rethink the whole approach.",
    "Love the focus on user experience over just technical metrics. Much more holistic view of the problem."
  ];
...
const evolveProblemStatement = (seedProblem, interactions) => {
  const evolutions = [
    ...
    seedProblem.replace("predict", "develop machine learning models to predict").replace("usage", "bandwidth usage patterns and peak demand cycles"),
    seedProblem.replace("routers", "WiFi access points and network infrastructure components").replace("upgrade", "proactively upgrade based on predictive analytics"),
    seedProblem + " This should include analyzing historical usage data, identifying seasonal patterns, and developing predictive models with at least 85% accuracy for 6-month forecasts.",
    
    // More comprehensive and stakeholder-focused
    `Building on ${seedProblem.toLowerCase()}, develop a comprehensive network capacity planning system that integrates usage forecasting, cost-benefit analysis, user experience metrics, and stakeholder communication protocols to ensure equitable access across all user groups.`,
    
    // Sustainability and innovation focused
    `Reframe ${seedProblem.toLowerCase()} as a sustainability-focused initiative that balances network performance optimization with energy efficiency, environmental impact reduction, and long-term infrastructure sustainability while maintaining user satisfaction.`,
    
    // User-centric and equity focused
    `Expand ${seedProblem.toLowerCase()} to prioritize network equity and user experience, ensuring that predictive upgrade decisions consider usage patterns across different user demographics, accessibility needs, and historical service disparities.`
  ];
...
const generateReasoning = (finalProblem, interactions, studyGroup) => {
  const reasoningTemplates = [
    `I chose this problem formulation because it addresses a critical infrastructure need while being technically feasible with available data. The AI feedback process was invaluable - the ${studyGroup === 'editor-first' ? 'editor' : 'challenger'} mode initially helped me ${studyGroup === 'editor-first' ? 'refine the technical specifications and identify concrete success metrics' : 'explore alternative perspectives and consider broader stakeholder impacts'}. The subsequent ${studyGroup === 'editor-first' ? 'challenger' : 'editor'} feedback then ${studyGroup === 'editor-first' ? 'opened my perspective to equity and sustainability considerations I hadn\'t initially considered' : 'helped me ground these creative ideas in measurable, actionable terms'}. This balanced approach ensures the solution is both innovative and practical, addressing immediate infrastructure needs while considering long-term organizational and social impacts.`,
    
    `Through the AI-assisted refinement process, I realized my initial problem statement was too narrow in scope. The iterative feedback helped me understand that effective data science problems require both technical rigor and creative problem-solving. The ${studyGroup === 'editor-first' ? 'editor feedback initially helped me specify concrete metrics and data requirements' : 'challenger prompts first pushed me to consider innovative approaches and alternative stakeholder perspectives'}, while the ${studyGroup === 'editor-first' ? 'challenger perspective later introduced alternative stakeholder viewpoints that enriched the problem framing' : 'editor feedback then ensured my expanded vision remained grounded in feasible methodologies'}. I ultimately chose this formulation because it demonstrates both analytical precision and creative thinking, essential qualities for impactful data science work that can drive real organizational change.`,
    
    `My problem evolution was significantly guided by the complementary nature of the two AI feedback modes. Starting with ${studyGroup === 'editor-first' ? 'editor feedback helped me tighten the technical specifications and identify concrete data sources' : 'challenger feedback pushed me to consider innovative approaches like fairness algorithms and community-driven solutions'}, which provided a solid foundation. The subsequent ${studyGroup === 'editor-first' ? 'challenger feedback then pushed me to consider broader implications and alternative approaches' : 'editor feedback helped me refine these creative ideas into measurable, actionable objectives'}. This final formulation represents a comprehensive approach that balances technical feasibility with social impact, addressing multiple stakeholder needs while maintaining methodological soundness. The problem now reflects both the precision needed for effective data science and the creativity required for meaningful innovation.`,
    
    `The AI feedback process fundamentally changed how I approach problem framing in data science. Initially, I focused primarily on ${studyGroup === 'editor-first' ? 'technical prediction accuracy' : 'conventional infrastructure optimization'}, but the iterative refinement process helped me develop a more holistic perspective. The ${studyGroup === 'editor-first' ? 'editor mode ensured my problem remained technically grounded with clear success metrics' : 'challenger mode introduced considerations of equity, sustainability, and user experience that I hadn\'t initially considered'}, while the ${studyGroup === 'editor-first' ? 'challenger mode expanded my thinking to include equity, user experience, and environmental considerations' : 'editor mode helped me translate these broader considerations into specific, measurable objectives'}. This final problem statement demonstrates that effective data science requires both analytical rigor and creative problem-solving, addressing not just technical challenges but also broader organizational and societal needs.`
  ];
...
const generateComprehensiveSyntheticData = async () => {
  console.log('🚀 Starting comprehensive synthetic data generation with OpenAI API...\n');
  
  await connectDB();
  
  // Clear existing synthetic data
  console.log('🧹 Clearing existing synthetic data...');
  await User.deleteMany({ sessionId: { $regex: /^study_participant_/ } });
  ...
  // Generate 60 users (2 per seed problem for better statistical power)
  ...
  console.log('\n📝 Generating problems and AI interactions with OpenAI API...');
  ...
  for (let userIndex = 0; userIndex < savedUsers.length; userIndex++) {
    ...
    console.log(`Processing user ${userIndex + 1}/${savedUsers.length}: ${user.sessionId.split('_')[2]}`);
    console.log(`  Seed problem: "${seedProblem.substring(0, 60)}..."`);
    
    // Generate problem start time (distributed across April 24 - May 2)
    ...
    const problem = {
      ...
      deviceInfo: {
        ...
        screenResolution: ["1920x1080", "1366x768", "1440x900", "1536x864"][Math.floor(Math.random() * 4)],
    ...
    // Generate first interaction with OpenAI API
    console.log(`  🔧 Generating ${firstPromptType} response with OpenAI...`);
    ...
    const firstRating = generateRating(firstPromptType, ['excellent', 'good', 'good', 'average'][Math.floor(Math.random() * 4)]);
    ...
    console.log(`  💡 Generating ${secondPromptType} response with OpenAI...`);
    ...
    const secondRating = generateRating(secondPromptType, ['excellent', 'good', 'good', 'average'][Math.floor(Math.random() * 4)]);
    ...
    const reasoning = generateReasoning(finalProblem, problem.interactions, user.studyGroup);
  ...
  console.log(`✅ Created ${savedProblems.length} problems with AI-generated interactions`);
  
  // Generate CSV export
  console.log('\n📊 Generating comprehensive CSV export...');
  ...
  const exportPath = join(__dirname, '..', 'exports', 'comprehensive_synthetic_data_april24_may2.csv');
  ...
  console.log('\n🎉 Comprehensive synthetic data generation complete!');
  console.log(`📁 CSV File: ${exportPath}`);
  console.log(`👥 Users: ${savedUsers.length}`);
  console.log(`📝 Problems: ${savedProblems.length}`);
  console.log(`🤖 AI Interactions: ${savedProblems.length * 2} (all generated with OpenAI API)`);
  ...
  const avgEditorRating = allResults.reduce((sum, r) => sum + r.editorRating.satisfaction, 0) / allResults.length;
  const avgChallengerRating = allResults.reduce((sum, r) => sum + r.challengerRating.satisfaction, 0) / allResults.length;
  ...
  console.log(`Average editor satisfaction: ${avgEditorRating.toFixed(2)}/5`);
  console.log(`Average challenger satisfaction: ${avgChallengerRating.toFixed(2)}/5`);
  console.log(`Seed problems used: ${seedProblems.length}`);
  
  // Show sample of generated data
  console.log('\n📝 Sample Generated Data:');
  const sample = allResults[0];
  console.log(`Sample Problem: "${sample.seedProblem.substring(0, 80)}..."`);
  console.log(`Final Problem: "${sample.finalProblem.substring(0, 80)}..."`);
...
// Run the generation
generateComprehensiveSyntheticData().catch(error => {
